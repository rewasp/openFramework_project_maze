...
void ofApp::draw() {
	...
	if (line_flag == TRUE)
		Mygraph->travel.draw(0, 0);


	if (shortest_flag == TRUE)
		Mygraph->shortest.draw(0, 0);
}
	...
	for (int i = 0; i < row; i++) {
		for (int j = 0; j < col; j++) {
			...
			if ((i % 2) && (j % 2 == 0)) {
				...
				no = ((i - 1) / 2) * col_num + (j - 1) / 2; // get room no.

				Mygraph->room_placement[no].set(line_x + (stride_x / 2), line_y + (stride_y / 2));
			}
...
void ofApp::read_from_file(ofFileDialogResult openFileResult)
{
	...
	/* For debugging.. check adj_list */
	//Mygraph->print_adj();

	Mygraph->dfs();
	Mygraph->find_shortest();
